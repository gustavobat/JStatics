...
// getQ - returns the external load vector.
...
// populateQ0 - calculates the known external loads and stores them in the vector of global forces Q.
...
/*void TStructure::populateDK(std::vector<TSupportDisplacement>& supportDisplacements)
{
...
// solve - solves for all of the above members in a single function.
void TStructure::solve(std::vector<TNodalLoad>& NodalLoads, std::vector<TDistributedLoad>& DistrLoads,
	std::vector<TElementEndMoment>& EndMoments, /*std::vector<TSupportDisplacement>& SupportDisplacements,*/
