...
void TLabel::paint(QPainter * Painter, const QStyleOptionGraphicsItem * Option, QWidget * Widget) 
{
	Painter->save();
...
void TLoadArrow::paint(QPainter * Painter,
	...
	if (fArrowStyle == Distributed) {
		...
		Painter->fillPath(filling, QBrush(QColor(Painter->pen().color()), Qt::SolidPattern));
...
QRectF TLoadArrow::boundingRect() const {
	// Draws a polygon from the arrow relevant points and transforms its coordinates 
	// into an appropriate coordinate system given by the transformation matrix.
...
void TMomentArrow::paint(QPainter * Painter,
	...
	QRectF boundary(fReferencePoint.x() - fArcRadius,
		fReferencePoint.y() - fArcRadius,
		2 * fArcRadius, 2 * fArcRadius);
	...
	Painter->fillPath(path, QBrush(QColor(Painter->pen().color()), Qt::SolidPattern));
...
TNodalLoadGraphics::TNodalLoadGraphics(QPointF LoadPosition, LoadType Type, bool Orientation) {
	qreal axisLength = 40;
	qreal tabLength = 7;
	qreal arcRadius = 25;
	...
	if (Type == Fx) {
		...
		fNodalLoad = new TLoadArrow(fLoadPosition, axisLength, tabLength, angle, TLoadArrow::ArrowStyle::Nodal);
	}
	else if (Type == Fy) {
		qreal angle;
		if (Orientation == true) {
		...
		fNodalLoad = new TMomentArrow(fLoadPosition, arcRadius, arcStartAngle, arcSpanAngle, tabLength, TMomentArrow::ArrowStyle::Nodal);
	}

	setFlag(ItemIsMovable, true);
}

TNodalLoadGraphics::~TNodalLoadGraphics() {
	delete fNodalLoad;
}
...
//------------------------------------------------------------------------------
TReactionLoadGraphics::TReactionLoadGraphics(QPointF LoadPosition, LoadType Type, bool Orientation) {
	qreal axisLength = 50;
	qreal tabLength = 10;
	qreal arcRadius = 30;
	qreal arcStartAngle = -45 * 16;
	...
	if (Type == Fx) {
		...
		fNodalLoad = new TLoadArrow(fLoadPosition, axisLength, tabLength, angle, TLoadArrow::ArrowStyle::Reaction);
	}
	else if (Type == Fy) {
		qreal angle;
		if (Orientation == true) {
		...
		fNodalLoad = new TMomentArrow(fLoadPosition, arcRadius, arcStartAngle, arcSpanAngle, tabLength, TMomentArrow::ArrowStyle::Reaction);
	}

	setFlag(ItemIsMovable, true);
}

TReactionLoadGraphics::~TReactionLoadGraphics() {
	delete fNodalLoad;
}
...
void TReactionLoadGraphics::paint(QPainter * Painter, const QStyleOptionGraphicsItem * Option, QWidget * Widget) {
	Painter->setPen(Qt::darkRed);
	fNodalLoad->paint(Painter, Option, Widget);
}

// TDistributedLoadGraphics default constructor.
TDistributedLoadGraphics::TDistributedLoadGraphics(TDistributedLoad * DistributedLoad, QPointF StartPoint, 
	QPointF EndPoint, qreal LoadScale) {
	...
	qreal xStep = cos(QLineF(StartPoint, EndPoint).angle() * PI / 180) * elementLength / numberOfArrows;
	qreal yStep = sin(QLineF(StartPoint, EndPoint).angle() * PI / 180) * elementLength / numberOfArrows;

	// Calculates the length of the extreme arrows proportionally.
	qreal arrow0Length, arrow1Length;
	if (abs(DistributedLoad->getNode0Load()) >= abs(DistributedLoad->getNode1Load())) 
	{
		arrow0Length = LoadScale * DistributedLoad->getNode0Load();
		arrow1Length = arrow0Length * DistributedLoad->getNode1Load() / DistributedLoad->getNode0Load();
	}
	else 
	{
		arrow1Length = LoadScale * DistributedLoad->getNode1Load();
		arrow0Length = arrow1Length * DistributedLoad->getNode0Load() / DistributedLoad->getNode1Load();
	}
	...
	for (int i = 0; i <= numberOfArrows; i++) 
	{
		...
		arrowLength = arrow0Length + (arrow1Length - arrow0Length) * i / numberOfArrows;

		fDistributedLoad.push_back(new TLoadArrow(referencePoint, arrowLength, arrowTabLength, angle, TLoadArrow::ArrowStyle::Distributed));
	}
}
...
QRectF TDistributedLoadGraphics::boundingRect() const 
{
	...
	polygon << fDistributedLoad[fDistributedLoad.size() - 1]->boundingRect().topLeft();
	polygon << fDistributedLoad[fDistributedLoad.size() - 1]->boundingRect().bottomRight();

	qreal extraBorder = 5.;
	return polygon.boundingRect().normalized().adjusted(-extraBorder, -extraBorder, extraBorder, extraBorder);
}

// TDistributedLoadGraphics painter function.
void TDistributedLoadGraphics::paint(QPainter * Painter, const QStyleOptionGraphicsItem * Option, QWidget * Widget) {

	// Changes pen color.
	QBrush supportsBrush(Qt::darkBlue, Qt::SolidPattern);
	Painter->setPen(QPen(supportsBrush, 1.2));

	for (int i = 0; i < (int)fDistributedLoad.size(); i++) 	{
		fDistributedLoad[i]->paint(Painter, Option, Widget);
	}
	...
	x1 = fEndPoint.x() + fDistributedLoad[fDistributedLoad.size() - 1]->getArrowLength() * cos(angle);
	y1 = fEndPoint.y() - fDistributedLoad[fDistributedLoad.size() - 1]->getArrowLength() * sin(angle);
...
TDiagramLoadGraphics::TDiagramLoadGraphics(QPointF StartPoint,
	QPointF EndPoint, double *Coefficients, qreal UScale) {
...
TDiagramLoadGraphics::~TDiagramLoadGraphics() {}
