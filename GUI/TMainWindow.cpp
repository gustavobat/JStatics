...
void TMainWindow::showJSONEditor() {
    fUi->jsonEditorWidget->show();
}
...
void TMainWindow::setCurrentFile(const QString& fileName) {
    ...
    if (fCurFile.isEmpty()) {
        ...
        populateAppliedLoadsScene(50);
        populateAxialForceDiagramScene(50);
        populateShearForceDiagramScene(50);
        populateBendingMomentDiagramScene(50);
...
void TMainWindow::on_saveChangesButton_clicked() {
    save();
}
    ...
    for (int i = 0; i < (int) supports.size(); i++) {
        ...
        if (supports[i].getFx() == true) {
            if (abs(reactions[CDOF]) > 0.00001) {
                ...
                labelMessage = QString::number(fabs(reactions[CDOF]), 'g', 3) + " kN";
        ...
        if (supports[i].getM() == true) {
            if (abs(reactions[CDOF]) > 0.00001) {
                ...
                labelMessage = QString::number(fabs(reactions[CDOF]), 'g', 3) + " kNm";
...
void TMainWindow::populateAppliedLoadsScene(qreal LoadScale) {
    ...
    std::vector<qreal> v(8, 0.0);

    //Maximum Load------------------------------------------------------------------------------------MEXI AQUI 
    if ((int) fDistributedLoads->size() != 0) {
        for (int i = 0; i < (int) fDistributedLoads->size(); i++) {
            if (fabs(fDistributedLoads->at(i).getNode0Load()) > v[0]) {
                v[0] = fabs(fDistributedLoads->at(i).getNode0Load());
            }
            if (fabs(fDistributedLoads->at(i).getNode0Load()) > v[1]) {
                v[1] = fabs(fDistributedLoads->at(i).getNode1Load());
            }
        }
    }

    if ((int) fNodalLoads->size() != 0) {
        for (int i = 0; i < (int) fNodalLoads->size(); i++) {
            if (fabs(fNodalLoads->at(i).getFx()) > v[4]) {
                v[4] = fabs(fNodalLoads->at(i).getFx());
            }

            if (fabs(fNodalLoads->at(i).getFy()) > v[5]) {
                v[5] = fabs(fNodalLoads->at(i).getFy());
            }
        }
    }
    ...
    for (int i = 0; i < (int) fDistributedLoads->size(); i++) {
        ...
        if (fDistributedLoads->at(i).getNode0Load() !=
            ...
            if (abs(fDistributedLoads->at(i).getNode0Load()) > 0.00001) {
                ...
                labelPos.rx() += (arrow0Length * 1.3) * cos(angle);
                labelPos.ry() -= (arrow0Length * 1.3) * sin(angle);

                labelMessage =
                        QString::number(fabs(fDistributedLoads->at(i).getNode0Load()), 'g',
                        3) +
                        " kN/m";
            ...
            if (abs(fDistributedLoads->at(i).getNode1Load()) > 0.00001) {
                ...
                labelPos.rx() += (arrow1Length * 1.3) * cos(angle);
                labelPos.ry() -= (arrow1Length * 1.3) * sin(angle);

                labelMessage =
                        QString::number(fabs(fDistributedLoads->at(i).getNode1Load()), 'g',
                        3) +
                        " kN/m";
                fAppliedLoads->addItem(
                        new TLabel(labelMessage, labelPos, 10, QPen(Qt::darkBlue)));
            ...
            labelPos.rx() += (arrow0Length * 1.2) * cos(angle);
            labelPos.ry() -= (arrow0Length * 1.2) * sin(angle);

            labelMessage = QString::number(
                    fabs(fDistributedLoads->at(i).getNode0Load()), 'g', 3) +
                    " kN/m";
            fAppliedLoads->addItem(
                    new TLabel(labelMessage, labelPos, 10, QPen(Qt::darkBlue)));
        ...
        fAppliedLoads->addItem(new TDistributedLoadGraphics(
                &(fDistributedLoads->at(i)), startPoint, endPoint, LoadScale)); //--------------------MUDANCA AQUI
    }
    ...
    for (int i = 0; i < (int) fNodalLoads->size(); i++) {
        ...
        if (fNodalLoads->at(i).getFx() != 0) {
            ...
            labelMessage =
                    QString::number(fabs(fNodalLoads->at(i).getFx()), 'g', 3) + " kN";
        ...
        if (fNodalLoads->at(i).getFy() != 0) {
            ...
            labelMessage =
                    QString::number(fabs(fNodalLoads->at(i).getFy()), 'g', 3) + " kN";
            fAppliedLoads->addItem(new TNodalLoadGraphics(
        ...
        if (fNodalLoads->at(i).getM() != 0) {
            ...
            labelMessage =
                    QString::number(fabs(fNodalLoads->at(i).getM()), 'g', 3) + " kNm";
            fAppliedLoads->addItem(new TNodalLoadGraphics(
...
void TMainWindow::populateAxialForceDiagramScene(qreal LoadScale) {
    ...
    //Maximum Axial Force-------------------------------------------------------------------------------MEXI AQUI
    double maxAF1 = 0; double maxAF2 = 0;
    for (int i = 0; i < (int) elements.size(); i++) {
        if(fabs((*fInternalLoads)[i][0]) > maxAF1){
            maxAF1 = fabs((*fInternalLoads)[i][0]);
        }
            if(fabs((*fInternalLoads)[i][3]) > maxAF2){
            maxAF2 = fabs((*fInternalLoads)[i][3]);
        }
    }
...
//        for(int k = 0;k < 4;k++){
//            std::cout << "Elemento: " << i << coefficients[k] << "\n" <<endl;
//        }
...
void TMainWindow::populateShearForceDiagramScene(qreal LoadScale) {
    ...
    //Maximum Shear Force-------------------------------------------------------------------------------MEXI AQUI
    double maxSF1 = 0; double maxSF2 = 0;
    for (int i = 0; i < (int) elements.size(); i++) {
        if(fabs((*fInternalLoads)[i][1]) > maxSF1){
            maxSF1 = fabs((*fInternalLoads)[i][1]);
        }
            if(fabs((*fInternalLoads)[i][4]) > maxSF2){
            maxSF2 = fabs((*fInternalLoads)[i][4]);
        }
    }
...
void TMainWindow::populateBendingMomentDiagramScene(qreal LoadScale) {
    ...
    //Maximum Moment------------------------------------------------------------------------------------------MEXI AQUI
    double maxM1 = 0;
    double maxM2 = 0;
    for (int i = 0; i < (int) elements.size(); i++) {
        if (fabs((*fInternalLoads)[i][1]) > maxM1) {
            maxM1 = fabs((*fInternalLoads)[i][2]);
        }
        if (fabs((*fInternalLoads)[i][4]) > maxM2) {
            maxM2 = fabs((*fInternalLoads)[i][5]);
        }
    }
